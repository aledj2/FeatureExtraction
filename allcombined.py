...
for designated regions of interest the probes that fall within this region are counted and any that fall into the abnormal category are counted. This is inserted into the relevant analysis table
...
class Analyse_array():
    ...
    def get_Z_scores(self, analysistable,ROI_ID,array_ID):
        ...
        Analyse_array().analyse_probe_Z_scores(array_ID,listofgreenZscores,listofredZscores,analysistable)
            

    def analyse_probe_Z_scores (self,arrayID,greenZscores,redZscores,analysistable):           
    ...
    def CompareHybPartners (self,table,arrayID): 
        array=arrayID
        roi=table
        ...
        try:
            ...
            db.rollback()
            print "fail - unable to access analysis table "+ str(roi) 
            if e[0]!= '###':
                raise
        finally:
            db.close()
        #produces a tuple where [0][0] is green del probes,[0][1]is Green_dup_probes_90 etc
