...
class extractData():  
    ...
    # for each feature firstly remove the \n using pop to remove the last item, replace and then append
        ...
        ins_feparams().insert_feparams(feparams,stats,features)
        

class ins_feparams():
    def insert_feparams(self,feparams_listin,stats_listin,features_listin):
        ...
        try:
            ...
            cursor.execute(feparams_ins_statement,(allfeparams[0],allfeparams[1],allfeparams[2],allfeparams[3],allfeparams[4],allfeparams[5],allfeparams[6],allfeparams[7],allfeparams[8],allfeparams[9],allfeparams[10],allfeparams[11],allfeparams[12],allfeparams[13],allfeparams[14],allfeparams[15],allfeparams[16],allfeparams[17],allfeparams[18],allfeparams[19],allfeparams[20],allfeparams[21],allfeparams[22],allfeparams[23],allfeparams[24],allfeparams[25],allfeparams[26],allfeparams[27],allfeparams[28],allfeparams[29],allfeparams[30],allfeparams[31],allfeparams[32],allfeparams[33],allfeparams[34],allfeparams[35],allfeparams[36],allfeparams[37]))
            db.commit()
...
class ins_stats():
    def insert_stats(self,statslist,array_ID,features_listin):
        #create a copy of the stats array created above using list().
        all_stats=list(statslist)
        arrayID=array_ID
...
class create_ins_statements():
    ...
    def looper(self,start,stop,allfeatures,arrayID):
        ...
        Array_ID=arrayID
        #loop through all features in range of lines given (provided when function is called below) 
...
class run_ins_statements:
    def run_ins_statements(self,features_listin,arrayID):       
        ...
        #Call the create insert statement class and name it features      
        featureslooper= create_ins_statements()
        #featureslooper.create_feat_ins(features_listin,arrayID)
        
        #call the looper function within this class and pass it the parameters from the class so that all 10 subgroups have been included
        featureslooper.looper(subset0,subset1,all_features,Array_ID)
        featureslooper.looper(subset1,subset2,all_features,Array_ID)
        featureslooper.looper(subset2,subset3,all_features,Array_ID)
        featureslooper.looper(subset3,subset4,all_features,Array_ID)
        featureslooper.looper(subset4,subset5,all_features,Array_ID)
        featureslooper.looper(subset5,subset6,all_features,Array_ID)
        featureslooper.looper(subset6,subset7,all_features,Array_ID)
        featureslooper.looper(subset7,subset8,all_features,Array_ID)
        featureslooper.looper(subset8,subset9,all_features,Array_ID)
        featureslooper.looper(subset9,no_of_probes,all_features,Array_ID)
        insert_features().insert_features(featureslooper.insertstatements,featureslooper.insertstatementnames)
        

#print  create_ins_statements.create_feat_ins.insertstatementnames
#create class which executes these SQL statements

class insert_features:
    def insert_features(self,insertstatements,insertstatementnames):
    #for each insert statement name in the insstatement list
