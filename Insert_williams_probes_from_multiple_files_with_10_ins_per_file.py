...
class Getfile():
    ...
    chosenfolder="F:\\arrayfiles\\williams"#USB
...
class ins_feparams():
    ...
    def insert_feparams(self,feparams_listin,stats_listin,features_listin,filein):
        ...
        cursor=db.cursor()
        feparams_ins_statement="""insert into williams_feparam (FileName,ProtocolName) values (%s,%s)"""
        try:           
            cursor.execute(feparams_ins_statement,(str(filename),allfeparams[0]))
            db.commit()
...
class ins_stats():
    def insert_stats(self,statslistin,array_ID,features_listin):
        ...
        runquery().runquery(features_listin,arrayID)
