...
class CalculateReferenceRange():
    def CalculateReference (self,go):
        go=go
        if go == 1:
            ...
            distinct_probe_names = "select distinct p.probename from probeorder p, features f where p.ProbeName=f.ProbeName"
            ...
            insert_average_sigint= "Insert into referencevalues set rsignalint=%s, gsignalint=%s, rsignalintSD=%s, gsignalintSD=%s where probename=%s"
            cursor=db.cursor()
