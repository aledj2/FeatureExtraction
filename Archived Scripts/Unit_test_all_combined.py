...
class Test():
    ...
    def test_get_target_probes(self):
        all_combined().get_list_of_target_probes()
        #check the list of target probes has been read correctly
        try:
            assert len(all_combined.list_of_probes) == 8391
            print "get target probes is ok"
        except:
...
#     
#     #after each row of features is amended check that the location is correct - nb need to check what 1st probe is!!
#     assert features[0][8]==
#     assert features[0][9]==
#     assert features[0][10]==
#          
#     #check data is removed from param
#     assert len(feparam)==38
#     
#     
